#!/usr/bin/env python3
"""
Generate ESM-2 embeddings for TEST data.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('esm2_test_embed.log'),
...
EMBEDDINGS_FILE = "test_esm2_embeddings.pkl"
...
def load_test_seqs():
    ...
    seqs = {}
    with open(TEST_SEQS, 'r') as f:
        pid = None
        for line in f:
            if line.startswith(">"):
                # Clean header: >P12345 ... -> P12345
                pid = line[1:].split()[0]
            else:
                if pid:
                    seqs[pid] = line.strip()
